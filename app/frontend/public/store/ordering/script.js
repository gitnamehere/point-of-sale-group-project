...
// cart map to store items in cart as item id: quantity
let cart = {};
    ...
    fetch(`/api/items?category=${id}`)
        ...
        .then((items) => {
            ...
            items.forEach((item) => {
                ...
                itemPrice.textContent = `$${parseFloat(item.price).toFixed(2)}`; // Ensure proper formatting
...
function addItemToCart(item) {
    ...
    if (cart[id]) {
        ...
        cart[id] = { quantity: 1, name, price: numericPrice }; // Store numeric price
        ...
        priceCell.textContent = numericPrice.toFixed(2); // Use numeric price
